...
namespace ShopMGR.Repositorios
{
    public class PasskeysRepositorio(ShopMGRDbContexto contexto) : IRepositorioPasskeys
    {
        ...
        {
            var credenciales = await _contexto.Passkeys
        ...
        {
            var credencialesExistentes = await _contexto.Passkeys
                .Where(pk => pk.IdUsuario == idUsuario)
                .Select(pk => pk.IdCredencial)
                .ToListAsync();
        ...
        {
            var passkey = await _contexto.Passkeys
                .Include(pk => pk.Usuario)
        ...
        {
            var challenge = await _contexto.Challenges
                .Where(pkc => pkc.ChallengeBase64 == challengeB64
                        && pkc.Tipo == "auth"
                        && pkc.FechaExpiracion > DateTime.Now)
                .FirstOrDefaultAsync();
        ...
        {
            var challenge = await _contexto.Challenges
                .Where(pkc => pkc.IdUsuario == idUsuario
                        && pkc.Tipo == "registro"
                        && pkc.FechaExpiracion > DateTime.Now)
