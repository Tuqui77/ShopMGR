...
public class MovimientoBalanceRepoositorio(ShopMGRDbContexto contexto)
    : IMovimientoBalanceRepositorio
{
    ...
    public async Task<List<MovimientoBalance>> ObtenerPorClienteAsync(int idCliente)
    {
        var movimientos = await _contexto
            .MovimientoBalance.Where(m => m.IdCliente == idCliente)
            .ToListAsync();

        return movimientos;
    }

    public Task<List<MovimientoBalance>> ObtenerPorClienteYPeriodoAsync(
        int idCliente,
        DateOnly desde,
        DateOnly hasta
    )
    {
        var movimientos = _contexto
            .MovimientoBalance.Where(m =>
                m.IdCliente == idCliente && m.Fecha > desde && m.Fecha < hasta
            )
            .ToListAsync();
    ...
    {
        var movimientosTrabajo = _contexto
            .MovimientoBalance.Where(m => m.IdTrabajo == idTrabajo)
            .ToListAsync();
