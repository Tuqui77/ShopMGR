    ...
    )
    {
        ...
        foreach (var assembly in assemblies)
        {
            var mappers = assembly
                ...
                .SelectMany(t =>
                    t.GetInterfaces() //Obtiene las interfaces que implementan
                        .Where(i =>
                            i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IMapper<,>)
                        ) //Se queda solo con las que implementan IMapper
                        .Select(i => new { Service = i, Implementation = t })
                ); //Devuelve pares de Service(IMapper<,>) y Implementation(los mappers de cada entidad)
